...
export function setupAuth(app: Express) {
  ...
  // Test endpoint for creating a test user with Firebase authentication
  // This is for development purposes only
  if (process.env.NODE_ENV === 'development') {
    app.get("/api/test/create-firebase-user", async (req, res, next) => {
      try {
        console.log("Test endpoint called: creating test Firebase user");
        
        // Set up test user data - you can modify these for testing
        const testFirebaseUid = `firebase-${Date.now()}`;
        const testUser = {
        ...
        const existingUser = await storage.getUserByFirebaseUid(testFirebaseUid);
        if (existingUser) {
          return res.status(200).json({
            message: "Test user already exists",
            user: existingUser,
            firebaseUid: testFirebaseUid,
            action: "Login with this Firebase UID"
          });
        }
        ...
        res.status(201).json({
          ...
          user: user,
          firebaseUid: testFirebaseUid,
          action: "Use this Firebase UID to login"
        });
      } catch (error) {
    ...
    app.get("/api/test/login-firebase-user", async (req, res, next) => {
      try {
        const testFirebaseUid = req.query.firebaseUid as string;
        
        if (!testFirebaseUid) {
          return res.status(400).json({
            message: "Missing firebaseUid query parameter",
            example: "/api/test/login-firebase-user?firebaseUid=firebase-123456789"
          });
        }
        
        console.log("Test login with Firebase UID:", testFirebaseUid);
        
        // Find the user
        const user = await storage.getUserByFirebaseUid(testFirebaseUid);
        if (!user) {
          return res.status(404).json({
            message: "Test user not found with this Firebase UID",
            firebaseUid: testFirebaseUid,
            action: "Create a test user first"
          });
        }
        ...
        req.login(user, (err) => {
          ...
          console.log("Test user logged in successfully:", user.id);
          const { password, ...userWithoutPassword } = user;
          res.status(200).json({
